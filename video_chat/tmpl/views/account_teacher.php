...
<div class="ki-live-video-conferences">
	...
	if(count($meetings)>0){
		?>
		<div class="ki-row">
			<center><a href="/account/?ki_vc_room=add_room&teacher=<?php echo get_current_user_id();?>" class="ki-but"><?php esc_html_e( 'Create a meeting request', 'ki-live-video-conferences' );?></a></center>
		</div>
		<?php
		foreach($meetings as $meeting){
			$post_id=$meeting->ID;
			
			$RoomDoctors=get_post_meta( $meeting->ID,'teachers', true );
